          ...
          {menuData.map((item) =>
            item.path ? (
              <Link key={item.id} href={item.path} passHref>
                <div className="flex flex-col items-center justify-center h-56 p-4 transition-all transform bg-white border border-gray-200 rounded-lg shadow-xl cursor-pointer hover:bg-gradient-to-r hover:text-white hover:shadow-lg hover:scale-105">
                  <div className="inline-flex items-center justify-center w-24 h-24 mb-3 text-indigo-500 bg-gray-100 rounded-full">
                    {item.icon}
                  </div>
                  <h2 className="font-serif text-xs text-gray-900 sm:text-lg">
                    {item.title}
                  </h2>
                </div>
              </Link>
